...
const ProductScreen = () => {
  ...
  // Get product data, loading, and error states directly from the Redux store
  const { product, loading, error } = useAppSelector((state) => state.products);

  // This useEffect dispatches the Redux action to fetch data
  useEffect(() => {
    // Ensure productId is not undefined before dispatching
    if (productId) {
      dispatch(fetchProductDetails(productId));
    }
  }, [productId, dispatch]);

  // This handler adds the item to the cart via Redux
